...
func strings_keys(dirs []string, yield func(path string, lc int, key string, value string, pre string, language string)) {
	...
	r := regexp.MustCompile("([^\\/]*)\\.lproj")

	for _, dir := range dirs {
		files, _ := glob(dir, ".strings")
		for _, file := range files {

			if filepath.Base(file) != "Localizable.strings" {
				continue
			}
			...
			if len(match) >= 2 {
				...
				// fmt.Println("reading strings file: ", file)
...
func xib_keys(dirs []string, yield func(path string, key string)) {
	...
	for _, dir := range dirs {
		...
		for _, file := range files {
			...
			fmt.Println("reading xib file: ", file)
...
func code_keys(dirs []string, yield func(path string, lc int, key string)) {
	...
	expressions := []*regexp.Regexp{
		regexp.MustCompile("NSLocalizedString\\(@\"(.*?)\","),
		regexp.MustCompile("NSLocalizedString\\(\"(.*?)\","),
		regexp.MustCompile("\"(.*?)\".localized"),
	}
...
func main() {
	...
	ignore := strings.Split(string(content), "\n")
	...
	for _, file := range files {
		...
		if !strings.HasPrefix(file, ".") {
			if !contains(ignore, filepath.Base(file)) {
